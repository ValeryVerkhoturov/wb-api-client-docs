...
generate.sh <ver>      →  clients/<lang>/…        (openapi-generator-cli, 5 языков)
    ...
    └── gen-readmes.py             (README.md на каждый язык)
     ↓ коммитится тегом v<version>, один тег покрывает все 5 языков
...
WB выпускает OpenAPI-спецификации. Написать 13 клиентов × 5 языков вручную — уплывут за неделю. Плюс генерации из спецификации: каждое поле, каждый enum, каждая обёртка ответа — ровно то, что задокументировал WB. И повторная синхронизация стоит нуля усилий после сборки пайплайна.
...
- Склеивает пер-спецификационные SDK в единое дерево на язык — Python-пакеты под `wb_api_client.<slug>`, TypeScript subpath-экспорты, Go под-пакеты, Java под-пакеты, PHP под-пространства.
...
Итог: `clients/{python,typescript,go,java,php}/` — пять пакетов, готовых к публикации.
...
PHP живёт в своём репозитории — [`ValeryVerkhoturov/wb-api-client-php`](https://github.com/ValeryVerkhoturov/wb-api-client-php) — подключённом сюда как git-submodule `clients/php`. Причина: Packagist требует `composer.json` в КОРНЕ обходимого репозитория. Ежедневный прогон коммитит и тегает оба репозитория с одним и тем же тегом.
